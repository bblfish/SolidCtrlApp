...
/**
  ...
  *
  * What do we need?
  * We need GET/POST/PUT/DELETE/QUERY
  * And we are *very* often interested in RDF Graphs being returned.
  * But we canno
  * Similar to an Http Request but where we sometimes want to be able to avoid
  * the serialisation part.
  *
  */
...
/** A response to a request. The Metadata tells if the response succeeded, what the problems may
  ...
  * Images, streams, ...)
  *
  * todo: should the result type be generic? In which case this would be a
  * generalisation of http4s.
  */
...
/**
  ...
  * @param headers Headers on the response
  *
  */
...
// type NamedGraphs = Map[Uri,Rdf#Graph]
  ...
  def get(key: Uri): Script[Response] =
    Free.liftF[LdpCmd, Response](Get[Response](key, identity))
