...
trait FoafWebTest[R <: RDF]()(using ops: Ops[R]) extends CatsEffectSuite {
  ...
  test("find bbl friends WebIDs inside graph (no jump)") {
    ...
    bblng.map { bblNg =>
      ...
      val kns: Seq[String] = (bblNg/foaf.knows).collect{
        case u: UriNGraph[R] => u.point.value
      }
  ...
  test("find canonical names of friends (as defined in their profile)") {
    www.getPNG(URI(Bbl)).flatMap { bblNg =>
      ...
      val names: fs2.Stream[IO, String] = (bblNg / foaf.knows).jump.collect {
        case ug: UriNGraph[R] => fs2.Stream(ug / foaf.name *)
      }.flatten.collect { case litG: LiteralNGraph[R] => litG.point.text }
      names.compile.toList.map(lst =>
        assertEquals(Set(lst*), Set(
        "Tim Berners-Lee", "Eric Prud'hommeaux", "Sarven Capadisli")))
    }
  }
  ...
  test("find all names of friends (local and remote)") {
    www.getPNG(URI(Bbl)).flatMap { bblNg =>
      //most of the names are only available from the definitional graphs
      val allNames: fs2.Stream[IO, String] = (bblNg / foaf.knows).jump.collect {
        case ug: SubjPNGraph[R] => fs2.Stream(ug / foaf.name *)
      }.flatten.collect { case litG: LiteralNGraph[R] => litG.point.text }
      
      allNames.compile.toList.map { lst =>
        assertEquals(Set(lst*), Set(
          "Tim Berners-Lee", "Eric Prud'hommeaux", "Sarven Capadisli", "James Gosling"))
      }
    }
  }
  ...
  test("find all names of friends (local and remote) - shorter version") {
    www.getPNG(URI(Bbl)).flatMap { bblNg =>
      //most of the names are only available from the definitional graphs
      val allNames: fs2.Stream[IO, String] =
        bblNg.rel(foaf.knows).jump
             .rel(foaf.name)
