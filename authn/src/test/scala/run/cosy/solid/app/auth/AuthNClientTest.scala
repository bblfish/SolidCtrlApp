...
class AuthNClientTest extends munit.CatsEffectSuite {
	...
	//
	{
		...
		test("BasicAuthentication should fail to respond to a request for non existent resource") {
			val req = Request[IO](
		...
		test("BasicAuthentication responds to authenticated non-existent resource with 404") {
			val req = Request[IO](
