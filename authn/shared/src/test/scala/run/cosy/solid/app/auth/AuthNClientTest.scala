...
class AuthNClientTest extends munit.CatsEffectSuite:
   ...
   def publicRoutes: HttpRoutes[IO] = HttpRoutes.of[IO] { case GET -> Root => Ok("Hello World") }
   ...
   def validatePassword(creds: BasicCredentials): IO[Option[String]] = IO.pure {
     if creds.username == username && creds.password == password then Some(creds.username)
     else None
   }
