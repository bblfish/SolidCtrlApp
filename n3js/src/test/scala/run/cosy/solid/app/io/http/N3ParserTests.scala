package run.cosy.solid.app.io.http
...
class N3ParserTests extends munit.FunSuite:
   ...
   val ntriples1: String = s"""<${bbl("#me")}> <${foaf("knows")}> <${w3c("timbl/card#i")}> .
     |<${bbl("#me")}> <${foaf("name")}> "Henry Story" .
     |""".stripMargin
   val ntriples2: String = s"""<${w3c("timbl/card#i")}> <${foaf("knows")}> <${bbl("#me")}> .
   ...
   test("Simple NTriples line Chunk Test") {
     ...
     val streamOfChunkStr: Stream[Pure, String] = chunkedStream.chunkN(3)
       .flatMap(cs => Stream.chunk(cs))
