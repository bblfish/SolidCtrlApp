...
/** + This is generated from the typescript module for N3 JS library, but we added a few methods and
  * intend to change the behavior. Lexer objects should therefore only be created with apply method
  * in the companion object.
  *
  * Because we need to rewrite some functions, we add the protected functions starting with an `_`.
  */
@JSImport("n3", "Lexer")
@js.native
